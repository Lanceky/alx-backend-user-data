...
def register_user(email: str, password: str) -> None:
    ...
    and the response payload matches expectations.
    """
...
def log_in(email: str, password: str) -> str:
    ...
    data = response.json()
    assert "email" in data and "message" in data, (
        f"Unexpected response payload: {data}"
    )
    assert data["email"] == email and data["message"] == "logged in", (
        f"Unexpected response payload: {data}"
    )
...
def reset_password_token(email: str) -> str:
    ...
    Assert the response status code is 200
    and the response payload contains a reset token.
